...
import { CHORDS, CHORD_GROUPS, type ChordShape } from "@/lib/chords";
...
export default function ChordsPage() {
  ...
  const play = async (key: string, chord: ChordShape) => {
    ...
    setActive(key);
    setTimeout(() => setActive((a) => (a === key ? null : a)), 700);
  };
  ...
  return (
    <div className="mx-auto max-w-3xl px-5 pt-16 pb-10 lg:px-10 lg:pt-10">
      ...
      <p className="mb-8 text-sm text-muted">
        The chords of worship guitar — tap any shape to hear it strummed on nylon strings.
      </p>

      {CHORD_GROUPS.map((group) => (
        <section key={group.label} className="mb-8">
          <h2 className="kicker mb-3 text-[13px] text-bronze">{group.label}</h2>
      ...
      <p className="mt-2 text-center text-xs text-faint">Tip: turn your volume up. On iPhone, the ring switch should be on.</p>
    </div>
  );
}
