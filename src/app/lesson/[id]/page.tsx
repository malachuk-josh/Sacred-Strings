...
const defaultSteps = [
  { title: "Watch & listen", body: "Play the demo above and hear how the shape should sound." },
  { title: "Try it slowly", body: "Copy the position one finger at a time. Accuracy before speed." },
  { title: "Play it through", body: "Repeat until it feels natural and every note rings clearly." },
];
...
export default function LessonPage() {
  ...
  const steps = lesson.steps ?? defaultSteps;
  ...
  return (
    <div className="min-h-screen">
        ...
        <div className="pb-32 pt-6">
          {mode === "Play Along" ? (
            <p className="text-sm leading-relaxed text-muted">
              Play along with the demo above at your own pace. When you can keep up smoothly, you&apos;ve got it — mark the lesson complete below.
            </p>
          ) : (
            <>
              <h2 className="mb-5 font-display text-[22px] font-semibold text-espresso">
                {mode === "Practice" ? "Practice it" : "How to play it"}
              </h2>
              {steps.map((s, i) => (
              ...
              <Link href="/metronome" className="mt-2 flex items-center gap-3 rounded-[14px] border border-border-warm-2 p-4" style={{ background: "#F3E7D4" }}>
                ...
                <div className="flex-1">
                  <div className="text-sm font-semibold text-[#3D2B1F]">Practice tempo</div>
                  <div className="text-xs text-muted">Start slow — 60 BPM</div>
                </div>
                <span className="text-[13px] font-bold text-bronze">Adjust</span>
              </Link>
            </>
          )}
        </div>
      ...
      {/* sticky CTA */}
      <div className="fixed inset-x-0 bottom-0 px-5 pb-24 pt-4 lg:bottom-0 lg:pb-6 lg:pl-72 lg:pr-10" style={{ background: "linear-gradient(180deg,rgba(250,244,234,0),#FAF4EA 40%)" }}>
